...
char*	AppTitle = "おおかみとひつじたち";			// ゲームのタイトル 
...
BOOL InitApp(HINSTANCE hInstance, int nCmdShow)									// ゲーム起動時の最初の初期化　必要な初期化を全てここに
{
	...
	MainFrame->ChangeScene(new sceneTutorial);		// チュートリアル用
#else
	MainFrame->ChangeScene(new Title());			// 初期シーン
#endif
	//MainFrame->ChangeScene(new sceneResult);		// 西田くん用
	return TRUE;
}
